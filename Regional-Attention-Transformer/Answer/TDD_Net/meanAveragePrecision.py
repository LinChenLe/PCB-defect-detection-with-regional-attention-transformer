...
for idx in range(len(gt_list)):
    ...
    for idx in range(len(match_mask)):
        ...
        # pdb.set_trace()
        category_mask = gt_category[match_gt_idx]+1 == submit_category[idx]
        ...
        if ToF == True:
            AP_info[gt_category[match_gt_idx].item()+1] += [1]
        elif ToF == False:
            AP_info[gt_category[match_gt_idx].item()+1] += [0]
...
plt.legend([f"missing_hole (AP={AP[1]:.04f})",
            f"mouse_bite (AP={AP[2]:.04f})",
            f"open_circuit (AP={AP[3]:.04f})",
            f"short (AP={AP[4]:.04f})",
            f"spur (AP={AP[5]:.04f})",
            f"spurious_copper (AP={AP[6]:.04f})"],loc='lower left')
plt.savefig("../../PR.pdf")