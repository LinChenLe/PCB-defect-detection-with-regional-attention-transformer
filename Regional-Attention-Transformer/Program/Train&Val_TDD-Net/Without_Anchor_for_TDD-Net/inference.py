...
torch.manual_seed(0)
category2name = {0:'hole',1:'bite',2:'open',3:'short',4:'spur',5:'copper',6:'nonOJ'}
# conf_th = 0.3
area_th = 0
class_th = 0.0
mask_th = 0.4
...
def inference(dataloader: data.Dataset, segmentation_model: nn.Module,
    ...
    for idx,(tested,tgt) in enumerate(dataloader):
        N,C,H,W = tested.shape
        tested = tested.to(device)

        for batch in range(N):
            tgt[batch]['category'] = tgt[batch]['category'].to(device)
        
        segmen_result = segmentation_model(tested)
        ...
        for batch in range(N):
            ...
            PC = len(maxidx)
            print(PC)
            img = tested[batch].permute(1,2,0).cpu().numpy()
