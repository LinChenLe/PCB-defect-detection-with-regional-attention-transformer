...
def inference(dataloader: data.Dataset, segmentation_model: nn.Module,
    ...
    for idx,(inp_img,tgt) in enumerate(dataloader):
        ...
        for batch in range(N):
            ...
            PC = len(maxidx)
            print(PC)
            img = inp_img[batch].permute(1,2,0).cpu().numpy()
