...
class creat_segmentation_main_model(nn.Module):
    ...
    def forward(self,template,tested):
        img = torch.cat([template,tested],dim=0)
        N,C,H,W = img.shape
        ...
        ly4feature_unbind = ly4feature.unbind(dim=0)
        ly4_temp_feature = torch.stack(ly4feature_unbind[:N//2],dim=0)
        ly4_test_feature = torch.stack(ly4feature_unbind[N//2:],dim=0)
        
        
        proposal_center,bbox,category = self.segmentation_model([ly4_temp_feature],
                                                                [ly4_test_feature])
        N = bbox.shape[0]
        result = []
...
class creat_segmentation_model(nn.Module):
    def __init__(self,num_object,device):
        ...
        ly4_num_channel = 512
        self.sub_norm4 = norm2D(ly4_num_channel)
    ...
    def forward(self,template_feature,test_feature):

        g4_temp_feature, = template_feature
        g4_test_feature, = test_feature
        g4_inp = self.sub_norm4(g4_test_feature-g4_temp_feature)
        
        N,C,H,W = g4_inp.shape

        encoder_inp = g4_inp
        encoder_out = self.Encoder(encoder_inp).flatten(-2,-1).permute(2,0,1)
        querires = g4_inp.flatten(-2,-1).permute(2,0,1)
        decoder_out = self.Dncoder(querires,
                                   encoder_out).permute(1,0,2)
        dx,dy = self.xy_L1(decoder_out).unbind(dim=-1)
        w,h = self.wh_L1(decoder_out).unbind(dim=-1)

        w = w.sigmoid()
        h = h.sigmoid()
        bbox = torch.stack([dx,dy,w,h],dim=-1)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
                     ...
                     pos: Optional[Tensor] = None):
        N,C,H,W = src.shape
        src = self.CBAM(src).flatten(-2,-1).permute(2,0,1)
        ...
        src = self.norm2(src).permute(1,2,0).contiguous().view(N,C,H,W)
        return src
