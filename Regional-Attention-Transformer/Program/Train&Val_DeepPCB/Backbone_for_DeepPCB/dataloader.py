...
class creat_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        Original_mask = torch.tensor((np.load(mask_path)/255).transpose(2,0,1),dtype=torch.float32)
        mask = Original_mask
        mask_category = torch.tensor(np.load(mask_category_path),dtype=torch.long)-1
...
# =============================================================================
        ...
        bbox = masks_to_boxes(mask[:-1]).to(torch.long)
        mask = torchvision.transforms.Resize((int(H/self.model_out_size),int(W//self.model_out_size)))(mask)
        mask[mask!=0] = 1.
        crop_img = torch.zeros([len(bbox),1,224,224],dtype=torch.float32)
        for seq in range(len(bbox)):
            h_2 = (bbox[seq,3]-bbox[seq,1])/2
            w_2 = (bbox[seq,2]-bbox[seq,0])/2
            # pdb.set_trace()
            crop_img[seq,0,math.floor(112-h_2):math.floor(112+h_2),math.floor(112-w_2):math.floor(112+w_2)]= \
                test_img[0,bbox[seq,1]:bbox[seq,3],bbox[seq,0]:bbox[seq,2]]
        assert torch.all(torch.sum(crop_img,dim=(-2,-1))!=0),"the crop img has some wrong, please check the dataloader.py"
        mask = torchvision.transforms.Resize((int(H/self.model_out_size),int(W//self.model_out_size)))(mask)

        return crop_img,mask_category
...
class DataLoader(data.DataLoader):
    ...
    def __iter__(self):
        ...
        for batchs in self.batch_sampler:
            ...
            categories = []
            for idx in batchs:
                crop_img,category = self.src_data[idx]
                crop_imgs +=[crop_img]
                categories += [category]
            crop_imgs = torch.cat(crop_imgs,dim=0)
            categories = torch.cat(categories,dim=0)
            yield crop_imgs,categories
    def __len__(self):
...
if __name__ == '__main__':
    ...
    for crop_img,mask_category in train_data:
        
        test = (crop_img.permute(0,2,3,1).repeat(1,1,1,3).numpy()*255).astype('uint8')[0].copy()
        H,W,C = test.shape
        # pdb.set_trace()

        # category = mask_category[0]['category']
